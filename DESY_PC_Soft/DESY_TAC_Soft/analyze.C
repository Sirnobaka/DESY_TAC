...
struct GenPoisParams{
  GenPoisParams():gain(0),u0(0),s0(0),sG(0),lambda(0),navg(0),npeaks(0),gain_spe_01(0),gs0(0),gs1(0),gm0(0),gm1(0){}
...
//    string outfile(filename);
    ofstream outfile;
    outfile.open((string(strtok(filename, ".")) + "_fitresults.tsv").c_str());
...
//    cout << "outfile: " << outfile << endl;
    ...
    for(int entry; entry < myTree->GetEntries(); entry++)
    {
        myTree->GetEntry(entry);
//        cout << "ADCsig: " << ADCsig << endl;
//        cout << "ADCtrig: " << ADCtrig << endl;
        if(ADCtrig == 4096) hmip->Fill(ADCsig);
    ...
    Double_t chisqr;
    Int_t    ndf;
    TCanvas *c1 = new TCanvas("c1","SPS",200,10,800,600);
    TCanvas *c2 = new TCanvas("c2","MIP",200,10,800,600);
//    hsps->Draw();
//
...
//
    c1->cd();
    TF1 *genp = GP(hsps);
    hsps->GetFunction("generpoiss")->SetLineColor(4);
    hsps->Draw();
//    histcl->Draw("sames");
//    histcl->SetLineWidth(0);
    outfile << "SPS_mu0" "\t" "SPS_sig0" "\t"  "SPS_Gain" "\t" "SPS_sGain" "\t" "SPS_Npe" <<
    "\t" "SPS_lambda" << endl;

    outfile << genp->GetParameter(4) << "\t" << genp->GetParameter(5) << "\t" <<
    genp->GetParameter(1) << "\t" << genp->GetParameter(2) << "\t" << genp->GetParameter(3) << "\t" << genp->GetParameter(0) << endl;
    
    outfile << "SPS_mean" "\t" "SPS_stdev" "\t" "SPS_skew" << endl;
    outfile << hsps->GetMean(1) << "\t" << hsps->GetStdDev(1) << "\t" << hsps->GetSkewness(1) << endl;
    
    outfile << "SPC_chi2" "\t" "SPS_NDF" << endl;
    outfile << genp->GetChisquare() << "\t" << genp->GetNDF() << endl << endl;
...
//
    ...
    gStyle->SetOptFit(1);
//
//    TCanvas *cmip = new TCanvas("MIP","MIP",1200,800);
//
        
    c2->cd();
    
    hmip->Rebin(4);
    TF1 *fitsnr1 = langaufit(hmip,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
    TF1 *fitmip = LandauGausFit(*hmip);
    fitsnr1->SetLineColor(4);
    ...
    outfile << fitmip->GetMaximumX() << "\t" << hmip->GetMean(1) << "\t" << hmip->GetStdDev(1) << "\t" << hmip->GetSkewness(1) << endl;
