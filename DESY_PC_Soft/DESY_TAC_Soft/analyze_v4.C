...
TF1* LandauGausFit(TH1F &hist)
{
  ...
  std::cout << "Last landau_gauss_nobound parameter[1]  = " << landau_gauss_function->GetParameter(1) << std::endl;
...
TF1 *GP(TH1F* hist2proc, int xlow=0, int xhigh=500){
  ...
  cout << "g1.GetChisquare = " << g1.GetChisquare() << endl
  << "g1.GetNDF = " << g1.GetNDF() << endl
  << "g2.GetChisquare = " << g2.GetChisquare() << endl
  << "g2.GetNDF = " << g2.GetNDF() << endl;
  fp.g0Chi2 = g1.GetChisquare();
  ...
  cout << "fp.gN0     = " << fp.gN0 << endl
  << "Integral 0 = " << hist2proc->Integral(fp.gm0-fp.gs0,fp.gm0+fp.gs0) << endl
  << "Integral Gauss 0 = " << g1.Integral(xpeaks[0]-(xpeaks[1]-xpeaks[0])/3,xpeaks[0]+(xpeaks[1]-xpeaks[0])/3) << endl
  << "fp.gN1     = " << fp.gN1 << endl
  << "Integral 1 = " << hist2proc->Integral(fp.gm1-fp.gs1,fp.gm1+fp.gs1) << endl
  << "Integral Gauss 1 = " << g2.Integral(xpeaks[1]-(xpeaks[1]-xpeaks[0])/3,xpeaks[1]+(xpeaks[1]-xpeaks[0])/3) << endl;
...
//    string outfile(strtok(filename, ".")) + "_fitresults.tsv";
    ...
    cout << "Loop over 'data' tree (" << myTree->GetEntries() << " entries)" << endl;
...
//
    ...
    //TF1 *genp = GP(hsps);
    TF1 *genp = new TF1(); 
    cout << "Before GP 1" << endl;
    genp = GP(hsps);
    cout << "GP is done" << endl;
    hsps->GetFunction("generpoiss")->SetLineColor(kGreen+2);
    cout << "GP is done 1" << endl;
    hsps->GetFunction("generpoiss")->SetLineWidth(2);
    cout << "GP is done 2" << endl;
    hsps->Draw("E");
    cout << "GP is done 3" << endl;
    hsps->GetXaxis()->SetRangeUser(0, 1000);

    //
    cout << "N entries of hsps hist = " << hsps->GetEntries() << endl;
    cout << "data from structure fp:" << endl
    << "Norm of gauss  0 = " << fp.gN0 << endl
    << "mean of gauss  0 = " << fp.gm0 << endl
    << "sigma of gauss 0 = " << fp.gs0 << endl
    << "Chi2/NDF gauss 0 = " << fp.g0Chi2 << " / " << fp.g0NDF << endl
    << "Integral in fit range   = " << fp.g0Integral << endl
    << "Integral in inf range   = " << fp.gN0 * fp.gs0 * sqrt(2*TMath::Pi()) << endl
    << "Integral from histogram = " << hsps->Integral(fp.gm0-(fp.gm1-fp.gm0)/2,fp.gm0+(fp.gm1-fp.gm0)/2) << endl
    << "Norm of gauss  1 = " << fp.gN1 << endl
    << "mean of gauss  1 = " << fp.gm1 << endl
    << "sigma of gauss 1 = " << fp.gs1 << endl
    << "Chi2/NDF gauss 1 = " << fp.g1Chi2 << " / " << fp.g1NDF << endl
    << "Integral in fit range   = " << fp.g1Integral << endl
    << "Integral in inf range   = " << fp.gN1 * fp.gs1 * sqrt(2*TMath::Pi()) << endl
    << "Integral from histogram = " << hsps->Integral(fp.gm1-(fp.gm1-fp.gm0)/2,fp.gm1+(fp.gm1-fp.gm0)/2) << endl;
    //
...
//
    ...
    //outfile << "MIP_MPV" "\t" "MIP_mean" "\t" "MIP_stdev" "\t" "MIP_skew" << endl;
    outfile << fitmip->GetMaximumX() << "\t" << hmip->GetMean(1) << "\t" << hmip->GetStdDev(1) << "\t" << hmip->GetSkewness(1) << "\t";

    //outfile << "MIP_chi2" "\t" "MIP_NDF" << endl;
    outfile << fitmip->GetChisquare() << "\t" << fitmip->GetNDF() << endl;
